...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        preBelief = self.beliefs

        # special case: ghost is captured
        if noisyDistance is None:
            P = self.getJailPosition()
            allPossible[P] = 1.0
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            # P(noisyDistance | TrueDistance) --> likelihood
            allPossible[p] = emissionModel[trueDistance] * preBelief[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                nextBelief[newPos] += prob * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        preBelief = self.getBeliefDistribution()

        # case1 : ghost is captured
        if noisyDistance is None:
            ...
            # case2 : all particles receive 0 weight
