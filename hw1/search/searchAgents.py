...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        visitedCorner = tuple()  # for hashing
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        ...
        for toFood in foodList:
            prob = PositionSearchProblem(problem.startingGameState, start=food, goal=toFood, warn=False, visualize=False)
            distance = len(search.bfs(prob))
            nextFoodDistance.append(distance)

    return min(distances) + max(nextFoodDistance)
    "*** YOUR CODE HERE ***"
    return 0
